...
final.grades <- function(gradebook, class = "GOV310L", non.enrolled = c("speede", "sm56684", "tm29778"), points = c(94, 90, 87, 84, 80, 77, 74, 70, 67, 64, 60), round = c("normal", "floor"), full.out = TRUE, final = c("points", "score")){
  ...
  names(gradebook) <- gsub(" ", ".", names(gradebook))

  # Creating a vector of scores based on points or score
  if(final %in% c("points", "point", "Points", "Point", "p", "P")){
    gradebook$final_num <- suppressWarnings(as.numeric(as.character(gradebook$Final.Points)))
  }else if(final %in% c("score", "scores", "Score", "score", "S", "s")){
    gradebook$final_num <- suppressWarnings(as.numeric(as.character(gradebook$Final.Score)))
  }else{
  ...
  if(round %in% c("floor", "Floor", "f", "F")){
  ...
  } else if(round %in% c("normal", "Normal", "n", "N")){
    ...
    stop("It appears that you forgot to set the round variable. Please set it to either 'normal' for noraml mathematical rounding of grades, or 'floor' to use only the whole number.")
  }
